...
<div class="container">
	<div class="card-body">
	<div class="topBt">
		<button onclick="location.href='dealerList.adm'; return false;">딜러관리</button>
	</div>
		<table class="table table-striped table-hover text-center">
			...
			<tbody>
				<c:if test="${expireDealer ne null}">
				<c:set var="startNo" value="${(pageInfo.page - 1) * 10 + 1}" />
					<c:forEach var="dealer" items="${expireDealer}" varStatus="status">
			            <tr>
			                <td id="item_no">${startNo + status.count-1}</td>
			                <td id="item_id">
			                	<a href="dealerDetailView.adm?user_id=${dealer.user_id}&display_num=2">${dealer.user_id}</a>
			                </td>
			                <td id="item_date"><fmt:formatDate pattern="yyyy/MM/dd" value="${dealer.user_expiredate}"/></td>			                		                
			            </tr>
			        </c:forEach>
			    </c:if>
				<c:if test="${empty expireDealer}">
		            <tr>
		                <td colspan="3">탈퇴딜러가 없습니다.</td>
		            </tr>
		        </c:if>
			</tbody>
		</table>
